...
```{r}
whole_text <- c("It was a bright cold day in April, and the clocks were striking thirteen. Winston Smith, his chin nuzzled into his breast in an effort to escape the vile wind, slipped quickly through the glass doors of Victory Mansions, though not quickly enough to prevent a swirl of gritty dust from entering along with him. 
...
```{r}
pattern <- ("[A-Z]+ [G-N]{3}")
whole_text_vodka <- str_replace_all(whole_text, pattern, replacement = "vodka")
str_extract_all(whole_text_vodka, "vodka")
```
...
###Step1: change`department` into ` section`
```{r}
whole_text_department <- whole_text %>% 
  str_replace_all("Department|department", "Section")
str_detect(whole_text_department, "Section")
```

###Step2: change `Ministry` into `Department`

```{r}

whole_text_ministry <- whole_text %>% 
  str_replace_all("Ministry", "Department")

str_detect(whole_text_ministry, "Department")


# attempt1
# pattern <- "[eartment]*"
# str_extract(whole_text, pattern)
# str_replace_all(whole_text, pattern, replacement = "Dep")

```

###Step3: Step3: change Newspeaks prefix from `Mini-` to `Dep-`
```{r}

pattern_np  <- "Mini(?!stry)"
whole_text_changed <- whole_text_ministry %>% 
  str_replace_all(pattern_np, "Dep")
str_detect(whole_text_changed,"[Dd]ep[a-z]+") #TRUE
str_detect(whole_text_changed,"Minitrue") #FALSE
str_detect(whole_text_changed,"Deptrue") #TRUE

#attempt1-
#"Mini(?!)stry+"

#attempt2
#pattern_np <- "[^Mini]+[(?i)stry$]+"
# whole_text_newspeak <- whole_text %>% 

#attempt3
# pattern_np  <- "Mini(?!stry)[a-z]+" 
# whole_text_changed <- whole_text_ministry %>% 
# str_replace_all(whole_text_ministry, pattern_np, replacement = "Dep")
# str_detect(whole_text,patter, negate = FALSE)
```

```{r}

whole_text %>% 
  str_extract_all("(?!)(Department | Ministry | Ministrie | Mini[a-z]+) ")
